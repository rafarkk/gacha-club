...
- `SLOT_DEFS` maps each slot (e.g. `hairBack`, `eyeL`, `sleeveR`, `propL`) to a part catalog `t` in `PARTS`. Left/right slots are linked via `PAIRS` (mirrored editing when `settings.linkPairs`).

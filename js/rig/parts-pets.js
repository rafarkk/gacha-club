...
function petDepth(k, svg) {
  ...
  return `<defs><filter id="${id}" x="-25%" y="-25%" width="150%" height="150%" color-interpolation-filters="sRGB">
    ...
    (k.shOp === 0 ? '' : `<ellipse cx="26" cy="54" rx="18" ry="4" fill="#0a0620" opacity="${k.shOp == null ? .6 : k.shOp}"/>`) + `<g class="fx" filter="url(#${id})">${svg}</g>`;
}
...
PARTS.pet = [null,
  ...
  /* ---- rodada 2 ---- */
  { n: 'Pato', d: k => `<path d="M8 48 Q4 30 22 30 Q32 30 36 40 Q48 36 50 44 Q48 56 26 56 Q10 56 8 48Z" fill="${k.c[0]}" ${po(k)}/><path d="M24 40 Q34 38 40 44" stroke="${k.c[2]}" stroke-width="1.4" fill="none" opacity=".5"/><circle cx="22" cy="20" r="13" fill="${k.c[0]}" ${po(k)}/>` + petEyes(k, 18, 26, 18, 2.4) + `<path d="M28 22 Q40 22 38 28 Q30 30 26 26Z" fill="#ffb52e" ${po(k, 1.8)}/>` + petCheeks(14, 30, 24, 2.4) },
  { n: 'Porquinho', d: k => `<path d="M11 50 Q8 34 26 32 Q44 34 41 50 Q26 54 11 50Z" fill="${k.c[0]}" ${po(k)}/>` + [[12, 14], [40, 14]].map(([x, y], i) => `<path d="M${x - 6} ${y + 6} L${x + (i ? 4 : -4)} ${y - 6} L${x + 6} ${y + 4}Z" fill="${k.c[0]}" ${po(k, 2)}/>`).join('') + `<path d="M26 12 C40 12 45 20 44 28 C43 36 36 40 26 40 C16 40 9 36 8 28 C7 20 12 12 26 12Z" fill="${k.c[0]}" ${po(k)}/><ellipse cx="26" cy="31" rx="7" ry="5" fill="${k.c[1]}" ${po(k, 1.6)}/><circle cx="23.5" cy="31" r="1.4" fill="${k.c[2]}"/><circle cx="28.5" cy="31" r="1.4" fill="${k.c[2]}"/>` + petEyes(k, 18, 34, 23, 2.6) + petCheeks(12, 40, 30) + paws(k, [19, 33], 51) },
  { n: 'Abelhinha', d: k => `<g class="anim-flap"><ellipse cx="18" cy="16" rx="8" ry="11" fill="#dff4ff" fill-opacity=".8" ${po(k, 1.6)}/><ellipse cx="34" cy="16" rx="8" ry="11" fill="#dff4ff" fill-opacity=".8" ${po(k, 1.6)}/></g><ellipse cx="26" cy="34" rx="18" ry="16" fill="${k.c[0]}" ${po(k)}/><path d="M12 30 Q26 26 40 30 M10 38 Q26 34 42 38" stroke="${k.c[2]}" stroke-width="5" fill="none"/>` + petEyes(k, 20, 32, 28, 2.4) + petCheeks(15, 37, 34, 2.6) + `<path d="M22 12 L18 2 M30 12 L34 2" stroke="${k.c[2]}" stroke-width="2"/>` },
  { n: 'Gatinho preto', d: k => `<path d="M38 44 Q54 44 52 30 Q51 22 45 24 Q49 28 47 34 Q45 40 36 40Z" fill="${k.c[2]}" ${po(k)}/><path d="M11 50 Q8 34 26 32 Q44 34 41 50 Q26 54 11 50Z" fill="${k.c[2]}" ${po(k)}/><path d="M9 22 L8 4 Q14 6 21 12Z M43 22 L44 4 Q38 6 31 12Z" fill="${k.c[2]}" ${po(k)}/><path d="M26 10 C42 10 46 20 45 27 C44 36 36 40 26 40 C16 40 8 36 7 27 C6 20 10 10 26 10Z" fill="${k.c[2]}" ${po(k)}/>` + [19, 33].map(x => `<ellipse cx="${x}" cy="25" rx="4" ry="5" fill="${k.c[0]}"/><ellipse cx="${x}" cy="25" rx="1.6" ry="4" fill="${k.c[2]}"/><circle cx="${x - 1.4}" cy="23" r="1.2" fill="#fff"/>`).join('') + petCheeks(13, 39, 31) },
  { n: 'Baleia', d: k => `<path d="M4 40 Q4 16 28 16 Q50 18 50 38 Q50 50 30 50 L8 50 Q4 48 4 40Z" fill="${k.c[0]}" ${po(k)}/><path d="M8 42 Q28 50 48 40 Q46 50 30 50 L10 50Z" fill="${light(k)}"/><path d="M50 32 L58 22 L58 40Z" fill="${k.c[0]}" ${po(k)}/>` + petEyes(k, 20, 20, 30, 2.6) + petCheeks(24, 24, 36, 2.6) + `<path d="M24 16 Q20 4 26 2 M24 16 Q30 6 34 8" stroke="#8fd3ff" stroke-width="2.6" fill="none" stroke-linecap="round"/>` },
  { n: 'Esquilo', d: k => `<path d="M34 50 Q58 40 48 14 Q40 2 30 12 Q44 18 40 32 Q36 44 30 46Z" fill="${k.c[0]}" ${po(k)}/><path d="M40 18 Q46 26 42 38" stroke="${k.c[1]}" stroke-width="4" fill="none"/><path d="M8 50 Q6 34 20 32 Q34 34 32 50 Q20 54 8 50Z" fill="${k.c[0]}" ${po(k)}/><circle cx="20" cy="22" r="12" fill="${k.c[0]}" ${po(k)}/><path d="M12 12 L10 4 L18 10Z M28 12 L30 4 L22 10Z" fill="${k.c[0]}" ${po(k, 1.6)}/>` + petEyes(k, 16, 24, 21, 2.2) + petCheeks(12, 28, 26, 2.4) + `<ellipse cx="20" cy="42" rx="5" ry="6" fill="#a8743a" ${po(k, 1.4)}/>` },
  { n: 'Leãozinho', d: k => blob({ c: k.c, F: k.c[1] }, [[12, 18, 8], [26, 10, 9], [40, 18, 8], [44, 30, 8], [8, 30, 8], [14, 40, 7], [38, 40, 7]]) + `<path d="M11 52 Q8 38 26 36 Q44 38 41 52 Q26 56 11 52Z" fill="${k.c[0]}" ${po(k)}/><circle cx="26" cy="28" r="13" fill="${k.c[0]}" ${po(k)}/>` + petEyes(k, 20, 32, 26, 2.4) + `<ellipse cx="26" cy="32" rx="2.4" ry="1.6" fill="${k.c[2]}"/>` + petMouth(k, 26, 35, 2) + petCheeks(15, 37, 32, 2.6) },
  { n: 'Arraia', d: k => `<path d="M26 18 Q52 24 52 34 Q40 38 26 50 Q12 38 0 34 Q0 24 26 18Z" fill="${k.c[0]}" ${po(k)}/><path d="M26 50 Q28 56 36 58" stroke="${k.c[2]}" stroke-width="2.4" fill="none"/>` + petEyes(k, 20, 32, 30, 2.4) + petCheeks(15, 37, 36, 2.6) + petMouth(k, 26, 38, 2) },
  { n: 'Gnomo', d: k => `<path d="M12 54 L14 36 L38 36 L40 54Z" fill="${k.c[1]}" ${po(k)}/><circle cx="26" cy="30" r="10" fill="#ffe0cc" ${po(k)}/><path d="M16 26 L26 -4 L36 26Z" fill="${k.c[0]}" ${po(k)}/><path d="M18 34 Q26 50 34 34 Q30 44 26 44 Q22 44 18 34Z" fill="#fff" ${po(k, 1.6)}/><ellipse cx="26" cy="33" rx="3" ry="2.4" fill="#ffb0a0"/>` + petEyes(k, 22, 30, 29, 1.8) },
  { n: 'Estrela-do-mar', d: k => `<path d="${Shape.star(26, 32, 24, 10)}" fill="${k.c[0]}" ${po(k)}/>` + [[26, 14], [12, 26], [40, 26], [18, 44], [34, 44]].map(([x, y]) => `<circle cx="${x}" cy="${y}" r="1.6" fill="${k.c[1]}"/>`).join('') + petEyes(k, 22, 30, 32, 2.2) + petMouth(k, 26, 38, 1.8) },
];
